...
mod status_trait_impls {
    ...
    use num_enum::{IntoPrimitive, TryFromPrimitive};
    use strum::{AsRefStr, IntoStaticStr, VariantArray};
    use sys::system_configuration::{kSCStatusFailed, kSCStatusInvalidArgument, kSCStatusOK};
    use crate::helpers::u32_into_u16_unchecked;
    use crate::status_temp::SCStatus;

    impl fmt::Debug for SCStatus {
        #[cold]
        fn fmt(&self, fmt: &mut fmt::Formatter<'_>) -> fmt::Result {
            let a = self.0.as_ref().map_or("Unknown", |s| s.as_ref());
...
//         }
//     }
//     impl TryFrom<OSStatus> for SCStatus {
//         type Error = OSStatus;
//
