...
use crate::helpers::u32_into_u8_unchecked;
...
pub enum SCStatusType {
    ...
    OK = u32_into_u8_unchecked(kSCStatusOK),
    /// A nonspecific failure occurred.
    Failed = u32_into_u8_unchecked(kSCStatusFailed),
    /// An invalid argument was specified.
    InvalidArgument = u32_into_u8_unchecked(kSCStatusInvalidArgument),
    /// Permission is denied; you must be root to obtain a lock. As a result, the function could not
    /// create or access preferences.
    AccessError = u32_into_u8_unchecked(kSCStatusAccessError),
    /// No such key.
    NoKey = u32_into_u8_unchecked(kSCStatusNoKey),
    /// Data associated with key already defined.
    KeyExists = u32_into_u8_unchecked(kSCStatusKeyExists),
    /// A lock is already held.
    Locked = u32_into_u8_unchecked(kSCStatusLocked),
    /// A lock is required for this operation.
    NeedLock = u32_into_u8_unchecked(kSCStatusNeedLock),
    /// The configuration daemon session is not active.
    NoStoreSession = u32_into_u8_unchecked(kSCStatusNoStoreSession),
    /// The configuration daemon is not available or no longer available.
    NoStoreServer = u32_into_u8_unchecked(kSCStatusNoStoreServer),
    /// Notifier is currently active.
    NotifierActive = u32_into_u8_unchecked(kSCStatusNotifierActive),
    /// The preferences session is not active.
    NoPrefsSession = u32_into_u8_unchecked(kSCStatusNoPrefsSession),
    /// A preferences update is currently in progress.
    PrefsBusy = u32_into_u8_unchecked(kSCStatusPrefsBusy),
    /// The configuration file cannot be found.
    NoConfigFile = u32_into_u8_unchecked(kSCStatusNoConfigFile),
    /// No such link exists.
    NoLink = u32_into_u8_unchecked(kSCStatusNoLink),
    /// A write was attempted on a stale version of the object.
    Stale = u32_into_u8_unchecked(kSCStatusStale),
    /// The maximum link count is exceeded.
    MaxLink = u32_into_u8_unchecked(kSCStatusMaxLink),
    /// Network reachability cannot be determined.
    ReachabilityUnknown = u32_into_u8_unchecked(kSCStatusReachabilityUnknown),
    /// Network service for the connection is not available.
    ConnectionNoService = u32_into_u8_unchecked(kSCStatusConnectionNoService),
    /// Network connection information is not available at this time.
    ConnectionIgnore = u32_into_u8_unchecked(kSCStatusConnectionIgnore),
}
